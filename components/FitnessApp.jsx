...
export default function App({user,supabase}){
  ...
  async function postToFeed(type,title,body){
    var sb=supabase||sbClient;
    try{
      await sb.from("feed_posts").insert({user_id:user.id,post_type:type,title:title,content:body});
  ...
  async function reactToPost(postId,reaction){
    var sb=supabase||sbClient;
    try{
      var{data:existing}=await sb.from("post_reactions").select("id").eq("post_id",postId).eq("user_id",user.id).eq("reaction",reaction).maybeSingle();
      if(existing){
        await sb.from("post_reactions").delete().eq("id",existing.id);
      }else{
        await sb.from("post_reactions").insert({post_id:postId,user_id:user.id,reaction:reaction});
      }
      loadFeed();
    }catch(e){loadFeed();}
  }
  ...
  async function saveW(){
    ...
    earnXP(75,"💪 Completed recorded workout!");
    checkAndUpdateStreak();
    if(user&&supabase){
      try{
        ...
        {tab==="social"&&(<div style={{display:"flex",flexDirection:"column",gap:11}}>
          ...
          {socialTab==="feed"&&(<div>
            ...
            {feed.map(function(post){
              ...
              var myReactions={};
              var totalFire=post.reaction_fire||0;
              var totalMuscle=post.reaction_muscle||0;
              var totalClap=post.reaction_clap||0;
